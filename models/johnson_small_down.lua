...
local function res_block()
  ...
  conv_block:add(backend.SpatialConvolution(n, n, 3, 3, 1, 1, 0, 0))
...
model:add(nn.SpatialAveragePooling(4,4,4,4,0,0))
model:add(pad(4, 4,4,4))
if params.add_noise then 
  model:add(backend.SpatialConvolution(6, n, 9, 9, 1, 1, 0, 0))
else
  model:add(backend.SpatialConvolution(3, n, 9, 9, 1, 1, 0, 0))
end

model:add(normalization(n))
...
model:add(nn.SpatialUpSamplingBilinear1(2))
-- model:add(nn.SpatialUpSamplingNearest(2))
model:add(pad(1, 1, 1, 1))
model:add(backend.SpatialConvolution(n, 64, 3, 3, 1, 1, 0, 0))
model:add(nn.ReLU(true))

-- model:add(nn.SpatialFullConvolution(n, 64, 3, 3, 2, 2, 1, 1, 1, 1))
...
-- model:add(nn.SpatialUpSamplingNearest(2))
model:add(pad(1, 1, 1, 1))
model:add(backend.SpatialConvolution(64, 32, 3, 3, 1, 1, 0, 0))
model:add(nn.ReLU(true))
